...
if ($result = mysqli_query($con, $sql)) {
    if (mysqli_num_rows($result) > 0) {
        ...
        echo "<th scope='col'>Institute</th>";
        echo "<th scope='col'>Batch</th>";
        echo "<th scope='col'>More</th>";
        echo "</tr>";
        while ($row = mysqli_fetch_array($result)) {
            echo "<tr scope='row'>";
            echo "<td scope='row'>"  . $row['fullname']  .  "</td>";
            echo "<td scope='row'>" .'<a href="student_desc.php?rollno='.$row["rollno"].'">'. "<img src=../images/students/".$row['image'].'  class="img-thumbnail"  width=100px height="100px">'.'</a>' . "</td>";
            echo "<td scope='row'>"  . $row['branch']  .  "</td>";
            echo "<td scope='row'>"  . $row['institute']  .  "</td>";
            echo "<td scope='row'>" . $row['batch'] . "</td>";
            echo "<td scope='row'>" . '<a href="student_desc.php?rollno='.$row["rollno"].'">'."<img src='../img/more1.png' width=20px height='20px'>".'</a>'. '</td>';
            echo "</tr>";
        }
        echo "</table>";
        // Free result set
